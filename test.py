...
class LandmineModel(Model):
    ...
    def __init__(self):
        ...
        self.running = True  # Simulation state
...
def draw_details(screen, model):
    ...
    details = [
        ...
        f"Destroyer State: {model.destroyer.state}",
...
def draw_buttons(screen, model):
    """Draw the start/pause buttons."""
    start_button_rect = pygame.Rect(SCREEN_SIZE + 10, SCREEN_HEIGHT - 60, 100, 40)
    pause_button_rect = pygame.Rect(SCREEN_SIZE + 120, SCREEN_HEIGHT - 60, 100, 40)

    # Change button color based on state
    pygame.draw.rect(screen, BUTTON_COLOR if model.running else BUTTON_HOVER_COLOR, start_button_rect)
    pygame.draw.rect(screen, BUTTON_COLOR if model.running else BUTTON_HOVER_COLOR, pause_button_rect)

    font = pygame.font.Font(None, 24)
    start_text = font.render("Start", True, TEXT_COLOR)
    pause_text = font.render("Pause", True, TEXT_COLOR)

    screen.blit(start_text, (start_button_rect.x + 25, start_button_rect.y + 10))
    screen.blit(pause_text, (pause_button_rect.x + 25, pause_button_rect.y + 10))

    return start_button_rect, pause_button_rect

def start():
    pygame.init()
    pygame.display.set_caption("Landmine Simulation with Start/Pause")
    ...
    while True:
        for event in pygame.event.get():
            ...
            if event.type == pygame.MOUSEBUTTONDOWN:
                ...
                start_button, pause_button = draw_buttons(screen, model)
                if start_button.collidepoint(mouse_pos) and not model.running:
                    model.toggle_running()  # Start the simulation
                elif pause_button.collidepoint(mouse_pos) and model.running:
                    model.toggle_running()  # Pause the simulation

        model.step()
        screen.blit(background_image, (0, 0))  # Draw the background image
        draw_grid(screen, model.grid, model)
        draw_details(screen, model)
        draw_buttons(screen, model)  # Draw the start/pause buttons
        pygame.display.flip()
        clock.tick(3)
